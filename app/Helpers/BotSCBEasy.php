...
class BotSCBEasy{
    ...
    public function __construct($username,$password,$bank_account){
        $this->cookie = public_path()."/scb/temp/scb.txt";
        if (!file_exists(public_path().'/scb/temp'.$bank_account)) {
            
            mkdir(public_path().'/scb/temp'.$bank_account, 0777, true);
        }
        $this->username = $username;
        $this->password = $password;

        $this->bank_account = $bank_account;
        $this->delete_files(public_path()."/scb/temp{$bank_account}/");
    ...
    private function curl($url,$method,$header = array(),$data = "",$res = false,$save = false, $file_name , $save_file = false){
        ...
        if($save_file){
            $fp = fopen(public_path()."/scb/temp{$this->bank_account}/".$file_name.".txt", "w+");
    ...
    private function Login($username,$password){
        ...
        $temp1 = file_get_contents(public_path()."/scb/temp{$this->bank_account}/temp_file_1.txt");
        ...
        if($SESSIONEASY1[0]){
            ...
            $temp2 = file_get_contents(public_path()."/scb/temp{$this->bank_account}/temp_file_2.txt");
            $SESSIONEASY2 = $this->DOMXPath($temp1,"//input[@name='SESSIONEASY']/@value");

            $file = fopen(public_path()."/scb/temp{$this->bank_account}/sessioneasy.txt","w+");
    ...
    private function getAccBnk(){
        $myfile = fopen(public_path()."/scb/temp{$this->bank_account}/sessioneasy.txt", "r") or die("Unable to open file!");
        $sessioneasy = fread($myfile,filesize(public_path()."/scb/temp{$this->bank_account}/sessioneasy.txt"));
        fclose($myfile);
        ...
        $temp_file_2 = file_get_contents(public_path()."/scb/temp{$this->bank_account}/temp_file_2.txt");
    ...
    public function Transaction(){
        $this->getAccBnk();
        @$bnk_tst = file_get_contents(public_path()."/scb/temp{$this->bank_account}/acc_bnk_tst.txt");
        $arr = [
            'status' => true,
            'transactions' => []
        ];
