...
class DepositController extends Controller
{
    ...
    public function index($brand) {
        ...
        $bank_account_reserve = BankAccount::whereBrandId($brand->id)-> first();

        if($brand->type_deposit == 1) {
            return view('gclub.members.deposit', compact('customer','brand','bank_accounts','promotions','bank_account_reserve'));
        } else {
            return view('gclub.members.deposit-fast', compact('customer','brand','bank_accounts','promotions','bank_account_reserve'));
        }
