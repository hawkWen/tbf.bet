...
class TransferController extends Controller
{
    ...
    public function index($brand) {
        ...
        return view('fastbet.transfer',\compact('brand'));
    ...
    public function withdraw(Request $request) {
        ...
        $response_withdraw = Bot::withdrawFastbet($brand,$customer,$input['amount']);

        // $customer->update([
        //     'credit' => $response_withdraw['data']['credit'],
        //     'last_update_credit' => date('Y-m-d H:i:s')
        // ]);

        if($response_withdraw['status'] === false) {

            return redirect()->back()->withErrors(['เครดิตไม่พอคุณมี เครดิตคงเหลือ '. $response_withdraw['data']['credit']]);
    ...
    public function view($brand,$line_user_id) {
        ...
        return view('fastbet.transfer-view', compact('customer','brand','bank_accounts','promotions','bank_account_reserve','balance'));
