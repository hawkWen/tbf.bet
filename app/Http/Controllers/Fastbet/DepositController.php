...
class DepositController extends Controller
{
    ...
    public function index($brand) {
        ...
        $bank_accounts = BankAccount::whereBrandId($brand->id)->whereStatusBot(1)->whereIn('type',[0,1,6,8])->get();
        ...
        foreach($promotion_queries as $promotion_query) {
            ...
            if ($promotion_query->type_promotion == 1) {
                ...
                $customer_deposits = CustomerDeposit::whereCustomerId($customer->id)->wherePromotionId($promotion_query->id)->whereBetween('created_at', [$dates[0], $dates[1]])->get();
        ...
        if($brand->type_deposit == 1) {
            return view('fastbet.members.deposit', compact('customer','brand','bank_accounts','promotions','bank_account_reserve','balance'));
        } else {
            return view('fastbet.members.deposit-fast', compact('customer','brand','bank_accounts','promotions','bank_account_reserve','balance'));
        }
