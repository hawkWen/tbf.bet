...
class HomeController extends Controller
{
    ...
    public function index() {
        ...
        if(isset($_GET['brand_id'])) {
            ...
            $bank_account_transactions = BankAccountTransaction::whereBrandId($_GET['brand_id'])->paginate(30)->appends(request()->except('page'));

        } else {

            $brand_select = '';

            $bank_account_transactions = BankAccountTransaction::paginate(30)->appends(request()->except('page'));

        }

        return view('admin.home', compact('bank_account_transactions','brands','brand_select'));
    }
    ...
    public function bankAccountTransactionStatus(Request $request) {
    ...
    public function cfOtp(Request $request) {
        ...
        $curl = curl_init();
        curl_setopt_array($curl, array(
        CURLOPT_URL => "https://scbcryp.ufa9545.com/pin/encrypt",
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => "",
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
        ...
        // $deviceId = $deviceId, token = $Auth2
            ...
            $data = array ('msg'=>'ข้อมูลไม่ถูกต้อง กรุณาลองใหม่02','status'=>500);
            echo json_encode($data);
        }
