...
function curl_scb($url,$post_data,$bank_account){
	sleep(1);
	$ch = curl_init($url);
    $sessionfile = 'scb/scb'.$bank_account.'.txt';
...
function get_otp_withdraw($scb_user,$scb_pass,$amount,$ac_number,$ac_bank,$mobile_otp,$bank_account){
	$data_display_curlerror = array("status"=>3,"detail"=>"SCB ตอบสนองช้าเกินไป โปรดลองใหม่ในอีก 1 นาที ");
    $sessionfile = 'scb/scb'.$bank_account.'.txt';
    $last_login=curl_scb("https://m.scbeasy.com/online/easynet/mobile/welcome.aspx",null,$bank_account);
	...
	if($runlogin){
		$firelogin = curl_scb("https://m.scbeasy.com/online/easynet/mobile/Default.aspx",null,$bank_account);
		if($firelogin=="error"){
			unlink($sessionfile);
		...
		$login=curl_scb($login_url,null,$bank_account);
		if($login=="error"){
			unlink($sessionfile);
			return $data_display_curlerror;
		}
		...
		$login=curl_scb($login_url,$logindata,$bank_account);
		if($login=="error"){
			unlink($sessionfile);
			return $data_display_curlerror;
        }
	...
	if($login_secess){
		if($ac_bank=="001"){//ธนาคารใน scb
			$formathbank=curl_scb("https://m.scbeasy.com/online/easynet/mobile/transfers/another-account-noProfile.aspx",null,$bank_account);
			...
			$formathbank=curl_scb("https://m.scbeasy.com/online/easynet/mobile/transfers/another-account-noProfile.aspx",$send_to_data,$bank_account);
			if($formathbank=="error"){
				unlink($sessionfile);
				return $data_display_curlerror;
			}
			$dataotp=curl_scb("https://m.scbeasy.com/online/easynet/mobile/transfers/another-account-confirm-noProfile.aspx",null,$bank_account);
			...
			$formathbank=curl_scb("https://m.scbeasy.com/online/easynet/mobile/transfers/another-bank-noProfile.aspx",null,$bank_account);
			if($formathbank=="error"){
				unlink($sessionfile);
				return $data_display_curlerror;
			}
			...
			$formathbank=curl_scb("https://m.scbeasy.com/online/easynet/mobile/transfers/another-bank-noProfile.aspx",$send_to_data,$bank_account);
			if($formathbank=="error"){
				unlink($sessionfile);
				return $data_display_curlerror;
			}
			$dataotp=curl_scb("https://m.scbeasy.com/online/easynet/mobile/transfers/another-bank-noProfile-confirm.aspx",null,$bank_account);
			$__VIEWSTATE=cutstring_scb($dataotp,'VIEWSTATE" value="','" /');
			$__VIEWSTATEGENERATOR=cutstring_scb($dataotp,'VIEWSTATEGENERATOR" value="','" /');
            $dataotp=iconv("windows-874","UTF-8",$dataotp);
            // print_r($dataotp);
            if(strpos($dataotp,"OTP")){
				$refer=trim(cutstring_scb($dataotp,'<span id="DataProcess_lbOTPRefNo">','</span>'));
				$acc_name=trim(cutstring_scb($dataotp,'ชื่อบัญชี:','</p>'));
                $result = array("status"=>1,"detail"=>"sended","acc_name"=>$acc_name,"refer" => $refer,"__VIEWSTATE"=>$__VIEWSTATE,"__VIEWSTATEGENERATOR"=>$__VIEWSTATEGENERATOR);
...
function confirm_otp($otp,$__VIEWSTATE,$__VIEWSTATEGENERATOR,$ac_bank,$bank_account){
	...
	if($ac_bank=="001"){//ธนาคารใน scb
		$return_otp_status=curl_scb("https://m.scbeasy.com/online/easynet/mobile/transfers/another-account-confirm-noProfile.aspx",$send_otp,$bank_account);
		$chotp_com=curl_scb("https://m.scbeasy.com/online/easynet/mobile/transfers/another-account-success-noProfile.aspx",null,$bank_account);
		...
		$return_otp_status=curl_scb("https://m.scbeasy.com/online/easynet/mobile/transfers/another-bank-noProfile-confirm.aspx",$send_otp,$bank_account);
		$chotp_com=curl_scb("https://m.scbeasy.com/online/easynet/mobile/transfers/another-bank-noProfile-success.aspx",null,$bank_account);
		$chotp_com=iconv("windows-874","UTF-8",$chotp_com);
		//if(strpos($return_otp_status,"success.aspx")){
